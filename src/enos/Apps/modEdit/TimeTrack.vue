<template>
  <g>
    ...
    <g v-if="index > (outputs.length - 1)">
      <text class="linker" @click="changeLabel" :height="ui.h" :x="0 + 50 + 20" :y="ui.y + 23 / 2">{{ metaItem.label }}</text>
      <!-- <text :height="socket.h" :x="socket.x" :y="socket.y + 23 / 2" fill="red">no-socket</text> -->
    </g>
    <g v-else>
      <text class="linker" @click="changeLabel" :height="ui.h" :x="0 + 50 + 20" :y="ui.y + 23 / 2">{{ metaItem.label }}</text>
      <!-- <text :height="socket.h" :x="socket.x" :y="socket.y + 23 / 2">socket{{ index }}</text> -->
    </g>

    <text class="linker" v-if="ii === 0" @click="remove" :height="ui.h" :x="5" :y="ui.y + 23 / 2">Remove</text>
    <text class="linker" v-if="ii === 1" @click="remove" :height="ui.h" :x="5" :y="ui.y + 23 / 2">Confirm</text>
